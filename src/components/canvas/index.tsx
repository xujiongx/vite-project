...
export function JSONCanvas({ content }: CanvasProps) {
  ...
  useEffect(() => {
    ...
    const zoomFn = zoom()
      ...
      .on(
        ...
        (event: {
          transform: {
            k: SetStateAction<number>;
            x: SetStateAction<number>;
            y: SetStateAction<number>;
          };
        }) => {
  ...
  return (
    <section
      ref={containerRef}
      className=' bg-silver-100 text-preussianBlue-900 relative h-full min-h-screen min-w-screen w-full'
      style={{
      ...
      <>
        ...
        {nodesRef.current?.map((node) => (
          <div
            id={node.id}
            key={node.id}
            className='border-silver-300 node active:text-preussianBlue-800 absolute overflow-hidden rounded border-[1px] border-solid bg-white p-2 transition-shadow duration-200 hover:cursor-grab active:cursor-grabbing active:shadow-md active:border-red-400'
            style={{
